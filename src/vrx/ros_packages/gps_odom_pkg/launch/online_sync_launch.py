...
def generate_launch_description():
	pkg_mr_robot_nav = get_package_share_directory('gps_odom_pkg')

	use_sim_time = LaunchConfiguration('use_sim_time')
	slam_params_file = LaunchConfiguration('slam_params_file')
	# with_rviz = LaunchConfiguration('with_rviz')
	#rviz_config_file = os.path.join(pkg_mr_robot_nav, 'rviz', 'slam_toolbox_mapping.rviz')
	...
	declare_slam_params_file = DeclareLaunchArgument('slam_params_file',
				default_value = os.path.join(pkg_mr_robot_nav, 'config', 'mapper_params_online_sync.yaml'),
