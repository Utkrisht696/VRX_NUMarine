...
class WaypointPublisher(Node):
    def __init__(self):
        ...
        self.buoy_position = (-33.722481945374248369, 150.674106898069908311)
    ...
    def order_waypoints_once(self):
        ...
        for i in range(len(self.ordered_waypoints) - 1):
            ...
            delta_lon_scaled = delta_lon * math.cos(math.radians(current_pose.position.x))
            yaw = math.atan2(delta_lat, delta_lon_scaled)  # Heading towards the next waypoint

            if self.last_yaw is not None:
                # Calculate the difference between the current yaw and the last yaw
                yaw_diff = yaw - self.last_yaw
                
                # Accumulate the yaw difference to prevent resetting at +-pi
                yaw = self.last_yaw + yaw_diff
