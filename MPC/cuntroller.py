...
from navigation import WAMV_Navigation
import rclpy
...
class WAMV_NMPC_Controller(Node):
    ...
    def __init__(self, config):
        ...
        self.state_pubb = self.create_subscription(Float64MultiArray, '/wamv/current_state', self.state_pubblisher, 10)
        ...
        self.navigation = WAMV_Navigation(config)
    ...
    def error_and_Jacobian(self, x, U, Xref, grad=False):
        ...
        for k in range(self.Np+1):
            ...
            if grad:
                ...
                x, dXdU = self.navigation.state_transition_U(x, ucur, self.dt, dXdU, k)
            else:
                x = self.navigation.state_transition_xonly(x, ucur, self.dt)
    ...
    def state_pubblisher(self,msg):
        self.current_state = np.array(msg.data).reshape((self.nx, 1))
