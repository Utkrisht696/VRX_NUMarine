...
class WAMV_NMPC_Controller(Node):
    ...
    def __init__(self):
        ...
        self.prev_yaw_ref = None
        self.yaw_offset_ref = 0
    ...
    def computeTrajectory(self, x):
        ...
        if distance_to_waypoint < 10.0:
            ...
            # Compute the difference between the current heading and the new heading

        ###    
        heading_diff = heading - x[2, 0]
        # Unwrap the heading to prevent discontinuity at +/- pi
    ...
    def control_loop(self):
        ...
        if self.waypoints is not None and self.gpsUpdates > 100 and self.imuUpdates > 100:
            ...
            # Check if both the position and heading match the desired trajectory
            distance_to_waypoint = np.linalg.norm(self.waypoints[0:2, [self.currentwaypoint]] - self.current_state[0:2, [0]])
            heading_error = np.abs(self.Xref[2, 0] - self.current_state[2, 0])

            # Unwrap the heading difference to account for discontinuities
            if heading_error > np.pi:
                heading_error = 2 * np.pi - heading_error

            # If close enough to both the waypoint position and heading
            if distance_to_waypoint < 0.5 and heading_error < 0.2:  # You can adjust the threshold for heading error
                self.currentwaypoint += 1
                print('Waypoint Completed!')
                print(self.currentwaypoint)
                if self.currentwaypoint >= self.waypoints.shape[1]:
    ...
    def reference_callback(self, msg):
        ...
        for i in range(len(msg.poses)):
            ...
            yaw = euler[2]
            # if self.prev_yaw_ref is not None:
            # # Calculate yaw difference between current and last yaw
            #     yaw_diff = yaw - self.prev_yaw_ref
            
            #     # Unwrap yaw to prevent it from jumping between +pi and -pi
            #     if yaw_diff > np.pi:
            #         self.yaw_offset_ref -= 2 * np.pi
            #     elif yaw_diff < -np.pi:
            #         self.yaw_offset_ref += 2 * np.pi
            
            # # Store unwrapped yaw
            # self.prev_yaw_ref = yaw
            # unwrapped_yaw = yaw + self.yaw_offset_ref
            self.waypoints[:3,i] = [x, y, yaw]  # yaw is euler[2]
